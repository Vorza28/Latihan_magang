...
<form action="{{ route('pembayaran.store') }}" method="POST" class="bg-white p-5 rounded shadow max-w-2xl">
    ...
    <label label class="block mb-2 font-semibold">Pilih Siswa</label>
    ...
    <div>
        ...
        <input type="number" name="tahun" class="border p-2 rounded w-full"
                value="{{ old('tahun', $defaultTahun) }}" required>
    </div>
    </div>
    ...
    <input type="date" name="tanggal_bayar" class="border p-2 rounded w-full" value="{{ old('tanggal_bayar') }}">
    ...
    <input type="text" name="keterangan" class="border p-2 rounded w-full" value="{{ old('keterangan') }}">
